...
class Secretaria:
    ...
    def encuentraPuntosFuera(self, listaDeConectados: list, listaDeTodos: list) -> list:
        ...
        for i in range(0, len(listaDeTodos)):
            ...
            for j in range(0, len(listaDeConectados)):
                if (
                    listaDeTodos[i].codigo == listaDeConectados[j].codigo
                    and listaDeTodos[i].tipo == listaDeConectados[j].tipo
                ):
                    flag = False
                    break
            if flag:
    ...
    def funcion(self, archivo1: str, archivo2: str):
        ...
        for i in range(0, len(self.listaFuera)):
            alfa.write(
                self.listaFuera[i].nombre
                + ";"
                + str(self.listaFuera[i].ubigeo)
                + ";"
                + str(self.listaFuera[i].longitud)
                + ";"
                + str(self.listaFuera[i].latitud)
                + ";"
                + str(self.listaFuera[i].alturaAntena)
                + ";"
                + self.listaFuera[i].tipo
                + ";"
                + str(self.listaFuera[i].metrosSobreElNivelDelMar)
                + "\n"
            )
