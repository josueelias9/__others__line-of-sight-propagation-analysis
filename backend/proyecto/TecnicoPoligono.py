...
class TecnicoPoligono:
    ...
    def llenaEstructuraMatricial(self):
        for i in range(
            0, self.myEstructura.n
        ):  # n veces, de 0 a n-1 (7 veces, de 0 a 6)
            # print('MsgTecnicoPoligono: iteracion {}'.format(i))
            lista, b = self.inter.traerInfoDeAlturas(
                self.myEstructura.puntoCero,
                self.myEstructura.estructuraLineaDeVista[i][self.myEstructura.m - 1],
            )

            # print(0'MsgTecnicoPoligono:{}-{}'.format(i,j))
            for j in range(
                0, self.myEstructura.m
            ):  # m veces, de 0 a m-1 (4 veces, de 0 a 3)
        ...
        while not len(lista2) == 0:
            for k in range(0, len(lista2)):
                ...
                if arriba == 1 or abajo == 1 or derecha == 1 or izquierda == 1:
                    if (
                        arriba == 1
                        and self.cierraPoligono(a0 + 1, a1) == False
                        and self.hayProblemaDiagonal(a0 + 1, a1) == False
                    ):
                    ...
                    if (
                        derecha == 1
                        and self.cierraPoligono(a0, a1 + 1) == False
                        and self.hayProblemaDiagonal(a0, a1 + 1) == False
                    ):
        ...
        while True:
            ...
            ####################################################
                ...
                if j + 1 == self.myEstructura.m:
                    ...
                    if self.pregunta_elPoligonoDaLaVuelta():
                        for p in range(0, len(listaBorrarq)):
                            if listaBorrarq[p][0] > int(self.myEstructura.n / 2) - 1:
                                self.myEstructura.estructuraMatricial[
                                    listaBorrarq[p][0]
                                ][listaBorrarq[p][1]] = 1
                        aux = []
            ...
            ####################################################
                ...
                if poligonito.finDelPoligono:
                    ...
                    if up == self.myEstructura.n:
                        poligonito.verSiSeLlegoAlFinalDelPoligonito(
                            [[i, left], [i, j], [0, j], [0, left], [i, left]]
                        )
                    else:
                        poligonito.verSiSeLlegoAlFinalDelPoligonito(
                            [[i, left], [i, j], [up, j], [up, left], [i, left]]
                        )
                    ESTADO_ACTUAL = ESTADO_FINAL
                elif DE == 3 and AB != 3:
    ...
    def cierraPoligono(self, i, j):
        ...
        if (
            (arriba == 2 or arriba == 3)
            and (abajo == 2 or abajo == 3)
            and (derecha == 0 or derecha == 1)
            and (izquierda == 0 or izquierda == 1)
        ):
            ...
            (arriba == 0 or arriba == 1)
            and (abajo == 0 or abajo == 1)
            and (derecha == 2 or derecha == 3)
            and (izquierda == 2 or izquierda == 3)
        ):
...
if __name__ == "__main__":
    ...
    imp.graficaPuntosEstructura(
        est.estructuraLineaDeVista, "out/TecnicoPoligono_puntos_1", "arrow"
    )
    imp.graficaPuntosEstructura(
        est.estructuraFigurasGeome, "out/TecnicoPoligono_puntos_2", "square"
    )
