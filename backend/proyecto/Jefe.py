...
class ArchivoUno:
    ...
    def __init__(
        self, numeroDeLDV, muestras, distancia, alturaTorreFantasma, lugarConectado
    ):
    ...
    def todasLasRelaciones(self):
        ...
        """
        # DEFINIENDO
        lista2 = self.tecnico.encontrarTodasLasRelacionesPosibles1LDVDist(
            self.listaTra, self.distancia
        )
        # SALIDA
    ...
    def generaClusteres(self):
        ...
        print(type(uno[0]))
        print(type(uno[1]))
        self.impresora.generaKmlRutas(
            uno, self.lugarConectado + "generaKmlRutas", "absolute"
        )
        self.impresora.generarTxtDeRelaciones(
            uno, self.lugarConectado + "generarTxtDeRelaciones"
        )
    ...
    def hasTodoSinInternet(self):
        ...
        for i in range(0, len(listaDePuntos)):
            ...
            est = Estructura(len(matrixAux), len(matrixAux[0]), 0.01, listaDePuntos[i])
            est.estructuraMatricial = matrixAux
            listaTotal = est.funcionMacroSinInternet()
            # tarea de impresora
            self.im.ploteadorDeMatriz(est.estructuraMatricial)
            self.im.hacerMallaSegunEstructuraMatricial(
                est.puntoCero.nombre + "_partido",
                est.estructuraMatricial,
                est.estructuraFigurasGeome,
                est.n,
                est.m,
            )
            self.im.graficaPuntosEstructura(
                est.estructuraLineaDeVista,
                est.puntoCero.nombre + "_puntos_linea_de_vista",
                "arrow",
            )
            self.im.graficaPuntosEstructura(
                est.estructuraFigurasGeome,
                est.puntoCero.nombre + "_puntos_geometria",
                "square",
            )
    ...
    def dePuntoAPoligono(self, punto: Punto):
        estructura = Estructura(
            self.numeroDeLDV,
            self.muestras,
            Factor.deKilometrosAGrados(self.distancia),
            punto,
            self.alturaTorreFantasma,
        )
    ...
    def ubicacionTorreFantasma(self, n):
        ...
        for j in range(len(self.listaTra) - n, len(self.listaTra)):
            ...
            for tupla in list(comb):
                print(
                    "MsgJefe: comb contiene varias tuplas. Una de ellas contiene los siguientes puntos:"
                )
                for i in range(0, len(tupla)):
                    print("elemento {}".format(tupla[i]))
                if funcion(tupla):
                    print(
                        "MsgJefe: Todos los puntos estan a menos de {}".format(
                            self.distancia * 2
                        )
                    )
                    a, b = hayInterseccionEnTupla(tupla)
                    if a == True:
                        self.impresora.deWktAKml(
                            self.lugarConectado + "deWktAKml", mapping(b)
                        )
                        self.impresora.generarTxtDePuntos(
                            tupla, self.lugarConectado + "generarTxtDePuntos"
                        )
                    ...
                    print(
                        "MsgJefe: Se salio porque hay un par de puntos que esta a mas de {}".format(
                            self.distancia * 2
                        )
                    )
        return False
    ...
    def interface(self):
        ...
        while b < 6:
            ...
            a = input(
                "OPCIONES\n1 {}\n2 {}\n: ".format(
                    self.lugarConectado, self.lugarNoConectado
                )
            )
            if a == "1":
...
class ArchivoDos:
    ...
    def generaArbol(self):
        ...
        uno, dos = self.tecnico.encuentraRelacionesPosibles(
            lista1, lista2, self.distancia
        )
    ...
    def puntosAlRededor(self, dd):
        ...
        :return:
        """
        nada, dentro, fuera = self.tecnico.encontrarTodasLasRelacionesPosibles2Dist(
            self.listaTra, self.listaAcc, dd
        )
        self.impresora.generaKmlPuntos(dentro, self.lugarConectado + "generaKmlPuntos")
        self.impresora.generarTxtDePuntos(
            dentro, self.lugarConectado + "generarTxtDePuntos"
        )
        self.impresora.generarTxtDeRelaciones(
            nada, self.lugarConectado + "generarTxtDeRelaciones"
        )
        ...
        for i in range(0, len(nodosNoConectados)):
            ...
            for j in range(0, len(listaAux)):
                ...
                if c.wkt != "GEOMETRYCOLLECTION EMPTY":
                    x = mapping(c)
                    self.impresora.deWktAKml(
                        listaAux[j].puntoInicial.nombre
                        + "-"
                        + listaAux[j].puntoFinal.nombre,
                        x,
                    )
                    print(
                        str(i)
                        + "-"
                        + str(j)
                        + " se encontro un area donde se puede poner un repetidor."
                    )
                    break
                else:
                    c = cAux
                    print(
                        "{}-{} estan a menos de {} km pero no se intersectan, prueba de nuevo...".format(
                            i, j, self.distancia * 2
                        )
                    )
