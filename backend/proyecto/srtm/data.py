...
class GeoElevationData:
    ...
    def __init__(
        self,
        srtm1_files: Dict[str, str],
        srtm3_files: Dict[str, str],
        file_handler: mod_utils.FileHandler,
    ...
    def get_elevation(
        self, latitude: float, longitude: float, approximate: bool = False
    ) -> Optional[float]:
        ...
        return geo_elevation_file.get_elevation(
            float(latitude), float(longitude), approximate
        )

    def _IDW(
        self, latitude: float, longitude: float, radius: float = 1
    ) -> Optional[float]:
        """
    ...
    def get_image(
        ...
        unknown_color: mod_utils.Color = mod_utils.Color(255, 255, 255, 255),
        zero_color: mod_utils.Color = mod_utils.Color(0, 0, 255, 255),
        min_color: mod_utils.Color = mod_utils.Color(0, 0, 0, 255),
        max_color: mod_utils.Color = mod_utils.Color(0, 255, 0, 255),
        mode: str = "image",
    ) -> Any:
        """
        ...
        if mode == "array":
            ...
            for row in range(height):
                for column in range(width):
                    latitude = latitude_from + float(row) / height * (
                        latitude_to - latitude_from
                    )
                    longitude = longitude_from + float(column) / width * (
            ...
            try:
                import Image as mod_image  # type: ignore
            except:
                from PIL import Image as mod_image  # type: ignore
            try:
                import ImageDraw as mod_imagedraw  # type: ignore
            except:
            ...
            for row in range(height):
                for column in range(width):
                    ...
                    if elevation == None:
                        ...
                        elevation_coef = (
                            (elevation or 0) - (min_elevation or 0)
                        ) / float(max_elevation)
                        if elevation_coef < 0:
                            elevation_coef = 0
                        if elevation_coef > 1:
                            elevation_coef = 1
                        color = mod_utils.get_color_between(
                            min_color, max_color, elevation_coef
                        )
                        if (elevation or 0) <= 0:
                            color = zero_color
    ...
    def add_elevations(
        ...
        if only_missing:
            for original_elevation, point in zip(
                original_elevations, list(gpx.walk(only_points=True))
            ):
                if original_elevation != None:
                    point.elevation = original_elevation

    def _add_interval_elevations(
        self, gpx: Any, min_interval_length: int = 100
    ) -> None:
        """
    ...
    def _add_sampled_elevations(self, gpx: Any) -> None:
        ...
        for point in gpx.walk(only_points=True):
            if (
                elevations_1[n] != None
                and elevations_2[n] != None
                and elevations_3[n] != None
            ):
...
class GeoElevationFile:
    ...
    def __init__(
        self, file_name: str, data: bytes, geo_elevation_data: GeoElevationData
    ) -> None:
    ...
    def approximation(self, latitude: float, longitude: float) -> Optional[float]:
        ...
        # here:
        importance_1 = d_meters - mod_utils.distance(
        ...
        importance_2 = d_meters - mod_utils.distance(
            latitude - d, longitude, latitude, longitude
        )
        ...
        importance_3 = d_meters - mod_utils.distance(
        ...
        importance_4 = d_meters - mod_utils.distance(
            latitude, longitude - d, latitude, longitude
        )
        ...
        if (
            elevation_1 is not None
            and elevation_2 is not None
            and elevation_3 is not None
            and elevation_4 is not None
        ):
            return (
    ...
    def _InverseDistanceWeighted(
        ...
        for offset in offsetmatrix:
            if (
                offset is not None
                and 0 <= row + offset[0] < self.square_side
                and 0 <= column + offset[1] < self.square_side
            ):
                cell = self.get_elevation_from_row_and_column(
                    int(row + offset[0]), int(column + offset[1])
                )
                if cell is not None:
                    # does not need to be meters, anything proportional
                    distance = mod_utils.distance(
                        latitude,
                        longitude,
