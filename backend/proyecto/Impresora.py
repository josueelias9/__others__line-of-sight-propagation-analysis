...
class Impresora:
    ...
    def generaKmlRutas(self, relacionDeRutas: list, nombre: str, tipo: str):
        ...
        for i in range(0, len(relacionDeRutas)):
            ...
            a2 = str(
                relacionDeRutas[i].puntoInicial.metrosSobreElNivelDelMar
                + relacionDeRutas[i].puntoInicial.alturaAntena
            )
            b0 = str(relacionDeRutas[i].puntoFinal.longitud)
            b1 = str(relacionDeRutas[i].puntoFinal.latitud)
            b2 = str(
                relacionDeRutas[i].puntoFinal.metrosSobreElNivelDelMar
                + relacionDeRutas[i].puntoFinal.alturaAntena
            )
            c = str(relacionDeRutas[i].distancia)[:5]
            f.write(
                "<Placemark>\n"
                "	<name>"
                + relacionDeRutas[i].puntoInicial.nombre
                + "->"
                + relacionDeRutas[i].puntoFinal.nombre
                + "</name>\n"
                "<description>"
                "inicio:\t" + a3 + "\n"
                "fin:\t" + b3 + "\n"
    ...
    def generaKmlPuntos(self, lista: list, nombre: str):
        ...
        for i in range(0, len(lista)):
            ...
            f.write(
                "		<Placemark>\n"
                "			<name>" + a3 + "</name>\n"
                "       <description>" + "altura:\t" + a2 + "\n"
                "identificador:\t" + a5 + "\n"
                "Tipo de antena:\t" + a4 + "       </description>\n"
                "			<LookAt>\n"
    ...
    def funcion(self, misPoligonos: Poligonos, myEstructura: Estructura):
        ...
        for i in range(0, len(misPoligonos.listaDePoligonitos)):
            ...
            for j in range(
                0, len(misPoligonos.listaDePoligonitos[i].listaDePuntosDelGranPoligono)
            ):
                a0 = misPoligonos.listaDePoligonitos[i].listaDePuntosDelGranPoligono[j][
                    0
                ]
                a1 = misPoligonos.listaDePoligonitos[i].listaDePuntosDelGranPoligono[j][
                    1
                ]
    ...
    def deWktAKml(self, nombre: str, diccionario: dict):
        ...
        if diccionario["type"] == "MultiPolygon":
            ...
            for i in range(0, len(diccionario["coordinates"])):
                ...
                for n in range(0, len(diccionario["coordinates"][i][0])):
                    f.write(
                        str(diccionario["coordinates"][i][0][n][0])
                        + ","
                        + str(diccionario["coordinates"][i][0][n][1])
                        + " "
                    )
                f.write(
                    "					</coordinates>\n"
                ...
                for j in range(1, len(diccionario["coordinates"][i])):
                    ...
                    for k in range(0, len(diccionario["coordinates"][i][j])):
                        f.write(
                            str(diccionario["coordinates"][i][j][k][0])
                            + ","
                            + str(diccionario["coordinates"][i][j][k][1])
                            + " "
                        )
                    f.write(
                        "					</coordinates>\n"
        ...
        if diccionario["type"] == "Polygon":
            ...
            for n in range(0, len(diccionario["coordinates"][0])):
                f.write(
                    str(diccionario["coordinates"][0][n][0])
                    + ","
                    + str(diccionario["coordinates"][0][n][1])
                    + " "
                )
            f.write(
                "					</coordinates>\n" "				</LinearRing>\n" "			</outerBoundaryIs>\n"
            ...
            for j in range(1, len(diccionario["coordinates"])):
                ...
                for k in range(0, len(diccionario["coordinates"][j])):
                    f.write(
                        str(diccionario["coordinates"][j][k][0])
                        + ","
                        + str(diccionario["coordinates"][j][k][1])
                        + " "
                    )
                f.write(
                    "					</coordinates>\n"
