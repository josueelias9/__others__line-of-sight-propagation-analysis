...
class TecnicoRed:
    ...
    def encuentraRelacionesPosibles(
        self, lista1: list, lista2: list, distancia: int
    ) -> (list, list):
        ...
        while True:
            for j in range(0, len(lista2)):
                ...
                if len(listaRelacionesAux) == 0:
                    ...
                    menorRelacion = self.calculaMenorDistanciaEnLista(
                        listaRelacionesAux
                    )
    ...
    def lineaDeVistaConsiderandoFresnel(
        self, listaDePuntos: list, puntoInicial: Punto, puntoFinal: Punto
    ) -> bool:
        ...
        for i in range(1, len(listaDePuntos) - 1):
            ...
            if (
                y_i - 0.6 * self.fresnel(puntoInicial, puntoFinal, listaDePuntos[i])
                <= listaDePuntos[i].metrosSobreElNivelDelMar
            ):
                return False
        return True
    ...
    def generaFresnel(self, puntoInicial: Punto, puntoFinal: Punto) -> list:
        fresnel = []
        listaDePuntos, estado = self.internet.traerInfoDePuntos(
    ...
    def generaLineaDeVista(self, puntoInicial: Punto, puntoFinal: Punto) -> list:
        ...
        for i in range(0, len(listaDePuntos)):
            x_i = i
            listaDePuntos[i].metrosSobreElNivelDelMar = ((y_f - y_0) / (x_f - x_0)) * (
                x_i - x_0
            ) + y_0
            fresnel.append(Relacion(listaDePuntos[i - 1], listaDePuntos[i]))
        return fresnel
    ...
    def encontrarTodasLasRelacionesPosibles1Dist(
        self, listaDePuntos: list, distanciaMaxima: int
    ) -> list:
        """
        :param listaDePuntos:
    ...
    def encontrarTodasLasRelacionesPosibles1LDVDist(
        self, listaDePuntos: list, distanciaMaxima: int
    ) -> list:
        """
        :param listaDePuntos:
    ...
    def encontrarTodasLasRelacionesPosibles2Dist(
        self, lista1: list, lista2: list, distanciaMaxima: int
    ) -> list:
        """
        :param lista1:
        :param distanciaMaxima: Distancia maxima de radio enlace
        ...
        return [
            listaDeRelacionesPosibles,
            list(listaConectadosLista1),
            list(listaConectadosLista2),
        ]

    def encontrarTodasLasRelacionesPosibles2LDV(
        self, lista1: list, lista2: list
    ) -> list:
        """
        :param lista1:
        :param distanciaMaxima: Distancia maxima de radio enlace
    ...
    def encontrarTodasLasRelacionesPosibles2LDVDist(
        self, lista1: list, lista2: list, distanciaMaxima: int
    ) -> list:
        """
        :param listaDePuntos:
    ...
    def lineaDeVista(self, punto1, punto2):
        try:
            ...
            a = False
            print(
                'MsgTecnicoRed: Estoy devolviendo "{}", pero hubo un error al usar los puntos "{}" y "{}"'.format(
                    a, punto1.nombre, punto2.nombre
                )
            )
        return a
    ...
    def unePuntosQueSeQuedaronFuera(self, lista, distancia):
        def tareaRepetitiva():
            ...
            for j in range(0, len(listaNoConectados)):
                ...
                for i in range(0, len(listaUltimosConectados)):
                    ...
                    if re.distancia < distancia and self.lineaDeVista(
                        listaNoConectados[j], listaUltimosConectados[i]
                    ):
        ...
        while len(listaNoConectados) != 0:
            ...
            while len(listaUltimosConectados) != 0:
                print(
                    "tamaño de listas: {}\t{}".format(
                        len(listaUltimosConectados), len(listaNoConectados)
                    )
                )
...
if __name__ == "__main__":
    """
    Hay que testear a esta clase individualmente para ver su funcionalidad.
    Las funciones dentro de funciones nos puede ayudar a resumir el codigo y hacerlo
    las leible. Buen punto
    """
    import Factor
    ...
    """creando objetos que pertenecen a clases que yo mismo cree"""
    sec = Secretaria(Factor.lugarDeTrabajo + "in/")
    imp = Impresora(Factor.lugarDeTrabajo)
    tec = TecnicoRed(muestras)
    """"""

    """lee archivo"""
    lista = sec.ordenameInput2(archivo)
    print("en total tengo {} puntos".format(len(lista)))
    """"""

    salida, salida2 = tec.unePuntosQueSeQuedaronFuera(lista.copy(), distancia)
    a = 0
    for i in range(0, len(salida)):
        print("########################")
        for j in range(0, len(salida[i].listaDeNodos)):
            print(salida[i].listaDeNodos[j])
        a = a + len(salida[i].listaDeNodos)
    print(a)
    imp.generaKmlPuntos(lista, archivo)
    imp.generaKmlRutas(salida2, "rutas", "absolute")

"""
pseudocodigo..de que? no lo se
listaTransporte
listaAcceso
listaRelacionesFinales
para cada uno de los elementos del listaAcceso
    listaRelacionesAux
    para cada uno de los elementos de listaTransporte
        si la distancia entre acceso y transporte es menor a 5 km y hay linea de vista
            agregar a relacion a listaRelacionesAux
        el caso contrario
            no hagas nada
    ahora que tienes todos los elementos de la listaTransporte que cumplen las condiciones
    busca cual de de todas las relaciones en listaRelacionesAux es la menor y agregalo
    a listaRelacionesFinales
manda a la imprimir listaRelacionesFinales
"""
